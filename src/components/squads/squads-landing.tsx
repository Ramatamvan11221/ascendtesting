...
const STYLES = `
  .sland-wrap{min-height:100vh;background:radial-gradient(ellipse at 50% 30%,rgba(245,158,11,0.05),transparent 60%),radial-gradient(ellipse at 80% 20%,rgba(249,115,22,0.03),transparent 50%),#070c14;display:flex;align-items:center;justify-content:center;padding:16px;position:relative;overflow:hidden;}
  ...
  .sland-orb-1{width:300px;height:300px;background:#f59e0b;top:-10%;left:-5%;animation:orbFloat1 15s ease-in-out infinite;}
  .sland-orb-2{width:220px;height:220px;background:#f97316;bottom:-8%;right:-5%;animation:orbFloat2 18s ease-in-out infinite;}
  ...
  .sland-card{max-width:520px;width:100%;text-align:center;padding:36px 20px;border-radius:24px;background:rgba(15,22,36,0.55);border:1px solid rgba(255,255,255,0.05);backdrop-filter:blur(24px);position:relative;z-index:10;}
  ...
  .sland-name{font-size:24px;font-weight:700;color:#edeff2;letter-spacing:-0.02em;margin-bottom:8px;}
  ...
  .badge-category{background:rgba(245,158,11,0.08);color:#f59e0b;border:1px solid rgba(245,158,11,0.15);}
  .badge-owner{background:rgba(249,115,22,0.06);color:#f97316;border:1px solid rgba(249,115,22,0.12);}

  .sland-goal{display:flex;align-items:center;gap:6px;justify-content:center;font-size:13px;color:#9aa4b8;margin-bottom:12px;font-weight:500;}
  @media(min-width:480px){.sland-goal{font-size:15px;}}
  .sland-desc{font-size:13px;color:#5a6478;line-height:1.7;margin-bottom:24px;max-width:420px;margin-left:auto;margin-right:auto;}
  @media(min-width:480px){.sland-desc{font-size:14px;margin-bottom:28px;}}

  .stats-row{display:flex;justify-content:center;gap:24px;margin-bottom:24px;padding:16px 0;border-top:1px solid rgba(255,255,255,0.04);border-bottom:1px solid rgba(255,255,255,0.04);}
  @media(min-width:480px){.stats-row{gap:40px;margin-bottom:32px;padding:20px 0;}}
  .stat-item{text-align:center;}
  .stat-value{font-size:20px;font-weight:700;color:#edeff2;letter-spacing:-0.02em;}
  @media(min-width:480px){.stat-value{font-size:24px;}}
  .stat-label{font-size:9px;color:#5a6478;text-transform:uppercase;letter-spacing:0.08em;margin-top:3px;font-weight:500;}
  ...
  .sland-back{display:inline-flex;align-items:center;gap:5px;color:#5a6478;font-size:12px;text-decoration:none;transition:color 0.3s;}
  @media(min-width:480px){.sland-back{font-size:13px;}}
  .sland-back:hover{color:#9aa4b8;}
`;
    ...
    <>
      ...
      <div className="sland-wrap">
        ...
        <motion.div className="sland-card" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.22, 0.61, 0.36, 1] }}>
          ...
          <p className="sland-goal"><Sparkles size={13} style={{ color: "#f59e0b" }} />{squadGoal}</p>
            ...
            <div className="stat-item">
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: "3px", marginTop: "5px" }}>
                {Array.from({ length: Math.min(roomCount, 4) }).map((_, i) => (
                  <Hash key={i} size={9} style={{ color: "#f59e0b", opacity: 0.5 }} />
                ))}
              </div>
            </div>
            <div className="stat-item">
              <p className="stat-value"><Crown size={16} style={{ display: "inline", color: "#f59e0b" }} /></p>
              <p className="stat-label">Owner</p>
              <p style={{ fontSize: "10px", color: "#5a6478", marginTop: "3px" }}>{ownerName.split(" ")[0]}</p>
            </div>
          </div>
          ...
          <div className="sland-join">
            {isLoggedIn ? (
              ...
              <Link href="/login" style={{ display: "inline-flex", padding: "12px 28px", borderRadius: "12px", background: "linear-gradient(135deg,#f59e0b,#f97316)", color: "#0a0a0a", fontSize: "14px", fontWeight: 600, textDecoration: "none", boxShadow: "0 12px 30px -10px rgba(245,158,11,0.35)", transition: "all 0.3s ease" }}>
