...
const STYLES = `
  .rc-header{display:flex;align-items:center;gap:10px;padding:14px 18px;border-bottom:1px solid var(--border-subtle);background:var(--bg-secondary);backdrop-filter:blur(12px);flex-shrink:0;}
  @media(min-width:480px){.rc-header{padding:14px 24px;}}
  .rc-header-name{font-size:15px;font-weight:600;color:var(--text-primary);}

  .members-wrap{flex:1;overflow-y:auto;}
  .members-subheader{display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid var(--border-subtle);flex-wrap:wrap;gap:8px;}
  @media(min-width:480px){.members-subheader{padding:14px 20px;}}
  .members-count{font-size:12px;color:var(--text-muted);font-weight:500;display:flex;align-items:center;gap:6px;}
  ...
  .member-card{display:flex;align-items:center;gap:10px;padding:11px 13px;border-radius:12px;border:1px solid var(--border-subtle);transition:all 0.25s;background:var(--bg-card);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);}
  @media(min-width:480px){.member-card{padding:13px 16px;border-radius:14px;}}
  .member-card:hover{background:var(--bg-card-hover);border-color:var(--border-medium);transform:translateY(-1px);}
  .member-avatar{width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:600;background:rgba(245,158,11,0.1);color:var(--amber);flex-shrink:0;border:2px solid transparent;transition:all 0.2s;}
  ...
  .member-goal{font-size:10px;color:var(--text-muted);margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
  ...
  .notif-item{display:flex;align-items:center;gap:10px;padding:10px 12px;border-radius:10px;border:1px solid var(--border-subtle);transition:all 0.2s;background:var(--bg-card);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);}
  ...
  .notif-icon.leave{background:rgba(239,68,68,0.1);color:var(--red);}
  .notif-icon.system{background:rgba(245,158,11,0.1);color:var(--amber);}
  ...
  .page-btn{width:28px;height:28px;border-radius:6px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all 0.2s;background:var(--bg-input);border:1px solid var(--border-subtle);color:var(--text-muted);font-family:'Inter',sans-serif;font-size:11px;}
  ...
  const renderMain = () => {
    ...
    if (activeView === "tasks") return (
      <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column" }}>
        <div className="rc-header"><Target size={18} style={{ color: "var(--amber)" }} /><span className="rc-header-name">Squad Tasks</span></div>
        <div style={{ padding: "20px" }}><SquadTasks squadId={squadId} isMember={true} /></div>
      </div>
    );
    if (activeView === "members") return (
      <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column" }}>
        <div className="rc-header"><Users size={18} style={{ color: "var(--amber)" }} /><span className="rc-header-name">Members</span></div>
        <div className="members-subheader">
          <span className="members-count"><Users size={13} style={{ color: "var(--amber)" }} />{localMembers.length} members</span>
        </div>
    ...
    if (activeView === "notifications") return (
      <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column" }}>
        <div className="rc-header"><Bell size={18} style={{ color: "var(--amber)" }} /><span className="rc-header-name">Notifications</span></div>
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", height: "100%", width: "100%" }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", height: "100%", background: "var(--bg-primary)" }}>{renderMain()}</div>
      </div>
    </>
  );
}