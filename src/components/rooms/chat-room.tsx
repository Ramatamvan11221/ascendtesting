...
const STYLES = `
  .cr-wrap{display:flex;flex-direction:column;height:100%;background:#0a0f18;}
  .cr-header{display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid rgba(255,255,255,0.04);background:rgba(10,15,24,0.8);backdrop-filter:blur(12px);flex-shrink:0;min-height:48px;}
  ...
  .cr-header-icon{color:#9aa4b8;flex-shrink:0;}
  .cr-header-name{font-size:14px;font-weight:600;color:#edeff2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
  @media(min-width:480px){.cr-header-name{font-size:15px;}}
  .cr-header-desc{font-size:10px;color:#5a6478;transition:color 0.3s;}
  @media(min-width:480px){.cr-header-desc{font-size:11px;}}
  .cr-header-desc.typing{color:#10b981;}
  .typing-dots{display:inline-flex;gap:2px;margin-left:2px;}
  .typing-dots span{width:3px;height:3px;border-radius:50%;background:#10b981;animation:typingBounce 1.4s infinite ease-in-out;}
  ...
  .cr-messages::-webkit-scrollbar-thumb{background:rgba(255,255,255,0.04);border-radius:2px;}
  ...
  .msg-group:hover{background:rgba(255,255,255,0.01);}
  @keyframes msgIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
  .msg-group.me{flex-direction:row-reverse;}
  .msg-avatar{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:600;flex-shrink:0;background:rgba(245,158,11,0.1);color:#f59e0b;margin-top:2px;}
  ...
  .msg-sender{font-size:10px;font-weight:600;color:#f59e0b;margin-bottom:1px;}
  @media(min-width:480px){.msg-sender{font-size:11px;margin-bottom:2px;}}
  .msg-sender.other{color:#818cf8;}
  .msg-bubble{padding:8px 12px;border-radius:14px;font-size:12px;line-height:1.5;color:#edeff2;word-wrap:break-word;width:fit-content;max-width:100%;}
  @media(min-width:480px){.msg-bubble{padding:10px 14px;border-radius:16px;font-size:13px;}}
  .msg-bubble.me{background:linear-gradient(135deg,#f59e0b,#f97316);color:#0a0a0a;border-bottom-right-radius:4px;margin-left:auto;}
  .msg-bubble.other{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.04);border-bottom-left-radius:4px;}
  .msg-bubble.deleted{background:rgba(255,255,255,0.015)!important;color:#5a6478!important;font-style:italic;border:1px dashed rgba(255,255,255,0.04)!important;}
  .msg-time{font-size:9px;color:#5a6478;margin-top:2px;}
  @media(min-width:480px){.msg-time{font-size:10px;margin-top:3px;}}
  .msg-time.me{text-align:right;}

  .cr-input-area{padding:10px 14px;border-top:1px solid rgba(255,255,255,0.04);flex-shrink:0;}
  ...
  .cr-input{flex:1;padding:9px 12px;border-radius:10px;border:1px solid rgba(255,255,255,0.06);background:rgba(255,255,255,0.02);color:#edeff2;font-size:12px;font-family:'Inter',sans-serif;outline:none;resize:none;max-height:80px;}
  ...
  .cr-input::placeholder{color:#3a4458;}
  .input-btn{width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;cursor:pointer;transition:all 0.2s ease;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.04);color:#5a6478;}
  @media(min-width:480px){.input-btn{width:38px;height:38px;border-radius:10px;}}
  .input-btn:hover{color:#f59e0b;border-color:rgba(245,158,11,0.2);}
  .send-btn{width:34px;height:34px;border-radius:10px;display:flex;align-items:center;justify-content:center;cursor:pointer;border:none;background:linear-gradient(135deg,#f59e0b,#f97316);color:#0a0a0a;transition:all 0.3s ease;}
  @media(min-width:480px){.send-btn{width:40px;height:40px;border-radius:12px;}}
  .send-btn:hover{transform:scale(1.05);}
  .send-btn:disabled{opacity:0.4;cursor:not-allowed;transform:none;}
  .empty-chat{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;color:#5a6478;gap:6px;padding:20px;}
  @keyframes spin{to{transform:rotate(360deg)}}
  .spinner{width:16px;height:16px;border:2px solid rgba(0,0,0,0.3);border-top-color:#0a0a0a;border-radius:50%;animation:spin 0.6s linear infinite;}
  ...
  .msg-file-name{font-size:11px;color:#edeff2;}
  @media(min-width:480px){.msg-file-name{font-size:12px;}}
  .msg-file-size{font-size:9px;color:#5a6478;}
      ...
      <div className="cr-wrap" style={{ position: "relative" }}>
        {contextMenu && (
          <>
            ...
            <div style={{ position: "fixed", left: Math.min(contextMenu.x, typeof window !== "undefined" ? window.innerWidth - 190 : 190), top: Math.min(contextMenu.y, typeof window !== "undefined" ? window.innerHeight - 140 : 140), zIndex: 100, background: "rgba(15,22,36,0.95)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "12px", padding: "4px", backdropFilter: "blur(20px)", minWidth: "170px", boxShadow: "0 16px 40px rgba(0,0,0,0.5)" }}>
              {contextMenu.allMine && (
                <button onClick={() => deleteMessages("everyone")} style={{ display: "flex", alignItems: "center", gap: "8px", width: "100%", padding: "9px 12px", borderRadius: "8px", border: "none", background: "none", color: "#ef4444", cursor: "pointer", fontSize: "12px", fontFamily: "'Inter', sans-serif" }}><Trash2 size={14} /> Delete for everyone</button>
              )}
              <button onClick={() => deleteMessages("me")} style={{ display: "flex", alignItems: "center", gap: "8px", width: "100%", padding: "9px 12px", borderRadius: "8px", border: "none", background: "none", color: "#edeff2", cursor: "pointer", fontSize: "12px", fontFamily: "'Inter', sans-serif" }}><Trash2 size={14} /> Delete for me</button>
              <div style={{ height: "1px", background: "rgba(255,255,255,0.04)", margin: "2px 6px" }} />
              <button onClick={() => setContextMenu(null)} style={{ display: "flex", alignItems: "center", gap: "8px", width: "100%", padding: "9px 12px", borderRadius: "8px", border: "none", background: "none", color: "#5a6478", cursor: "pointer", fontSize: "12px", fontFamily: "'Inter', sans-serif" }}><X size={14} /> Cancel</button>
            </div>
          </>
        )}
        ...
        <div className="cr-header">
          ...
          <button style={{ background: "none", border: "none", color: "#5a6478", cursor: "pointer", padding: "4px" }}><MoreVertical size={16} /></button>
        </div>

        {loading ? (
          <div className="empty-chat"><Loader2 size={22} className="spinner" style={{ borderTopColor: "#f59e0b", borderColor: "rgba(255,255,255,0.1)" }} /></div>
        ) : messages.length === 0 ? (
                      ...
                      <div key={msg.id}>
                        {msg.fileUrl && (msg.fileUrl.match(/\.(jpg|jpeg|png|gif|webp)/i) || msg.fileUrl.includes("ibb.co")) ? (
                          ...
                          <a href={msg.fileUrl} download={msg.fileName} className="msg-file" target="_blank" rel="noopener noreferrer">
                            <Paperclip size={14} style={{ color: "#f59e0b" }} /><div><p className="msg-file-name">{msg.fileName}</p><p className="msg-file-size">{msg.fileSize}</p></div>
                          </a>
                        ) : (
        ...
        <div className="cr-input-area">
          <div className="cr-input-row">
            <button className="input-btn" onClick={() => fileInputRef.current?.click()} disabled={uploading}>
              {uploading ? <Loader2 size={14} style={{ borderTopColor: "#f59e0b", animation: "spin 0.6s linear infinite" }} /> : <Image size={14} />}
            </button>
