...
const STYLES = `
  .dash-wrap { min-height: 100vh; background: radial-gradient(ellipse at 50% 0%, rgba(245,158,11,0.04), transparent 60%), #070c14; padding: 24px 16px; position: relative; }
  ...
  .welcome-name { font-size: clamp(22px, 4vw, 34px); font-weight: 700; color: #edeff2; letter-spacing: -0.02em; }
  .welcome-goal { font-size: 13px; color: #9aa4b8; display: flex; align-items: center; gap: 6px; margin-top: 4px; flex-wrap: wrap; }
  ...
  .badge-commit { background: rgba(245,158,11,0.08); color: #f59e0b; border: 1px solid rgba(245,158,11,0.18); }
  ...
  .qa-card {
    ...
    background: rgba(15,22,36,0.5); border: 1px solid rgba(255,255,255,0.04);
    transition: all 0.35s cubic-bezier(0.22,0.61,0.36,1);
    backdrop-filter: blur(16px); flex: 1; min-width: 0;
  }
  @media (min-width: 480px) { .qa-card { padding: 18px 22px; border-radius: 18px; } }
  .qa-card:hover { border-color: rgba(245,158,11,0.3); background: rgba(20,30,48,0.6); transform: translateY(-3px); box-shadow: 0 16px 40px -16px rgba(0,0,0,0.4); }
  .qa-icon {
  ...
  .qa-label { font-size: 13px; font-weight: 600; color: #edeff2; margin-bottom: 1px; }
  @media (min-width: 480px) { .qa-label { font-size: 14px; } }
  .qa-sub { font-size: 11px; color: #5a6478; }
  ...
  .stat-label { font-size: 10px; color: #5a6478; text-transform: uppercase; letter-spacing: 0.06em; margin-top: 4px; font-weight: 500; }
  @media (min-width: 480px) { .stat-label { font-size: 11px; } }

  /* Squad */
  .squad-section { border-radius: 18px; background: rgba(15,22,36,0.5); border: 1px solid rgba(255,255,255,0.04); backdrop-filter: blur(14px); overflow: hidden; }
  @media (min-width: 480px) { .squad-section { border-radius: 22px; } }
  .squad-header { display: flex; align-items: center; justify-content: space-between; padding: 16px 18px; border-bottom: 1px solid rgba(255,255,255,0.04); }
  @media (min-width: 480px) { .squad-header { padding: 20px 24px; } }
  .squad-title { font-size: 15px; font-weight: 600; color: #edeff2; display: flex; align-items: center; gap: 8px; }
  @media (min-width: 480px) { .squad-title { font-size: 17px; } }
  .squad-create { display: flex; align-items: center; gap: 5px; padding: 7px 14px; border-radius: 10px; background: rgba(245,158,11,0.08); border: 1px solid rgba(245,158,11,0.15); color: #f59e0b; text-decoration: none; font-size: 12px; font-weight: 500; transition: all 0.3s; white-space: nowrap; }
  ...
  .squad-card { display: flex; align-items: center; gap: 12px; padding: 14px 18px; text-decoration: none; cursor: pointer; transition: all 0.25s; position: relative; border-bottom: 1px solid rgba(255,255,255,0.02); }
  @media (min-width: 480px) { .squad-card { padding: 18px 24px; } }
  @media (min-width: 640px) { .squad-card { border-right: 1px solid rgba(255,255,255,0.02); } .squad-card:nth-child(even) { border-right: none; } }
  ...
  .squad-card::before { content: ''; position: absolute; left: 0; top: 20%; bottom: 20%; width: 2px; background: #f59e0b; opacity: 0; border-radius: 0 2px 2px 0; transition: opacity 0.3s; }
  ...
  .squad-name { font-size: 13px; font-weight: 600; color: #edeff2; }
  @media (min-width: 480px) { .squad-name { font-size: 14px; } }
  .squad-goal { font-size: 11px; color: #5a6478; margin-top: 1px; }
  .squad-meta { font-size: 10px; color: #5a6478; display: flex; align-items: center; gap: 4px; margin-top: 3px; }
  .squad-arrow { color: #3a4458; transition: all 0.3s; flex-shrink: 0; margin-left: auto; }
  ...
  .empty-icon { color: #2a3448; margin-bottom: 12px; }
  .empty-text { font-size: 13px; color: #5a6478; margin-bottom: 16px; }
  .empty-btn { display: inline-flex; align-items: center; gap: 6px; padding: 10px 20px; border-radius: 10px; border: 1px solid rgba(245,158,11,0.2); color: #f59e0b; text-decoration: none; font-size: 12px; font-weight: 500; transition: all 0.3s; background: rgba(245,158,11,0.04); }
...
export function DashboardContent({
  ...
  const quickActions = [
    { href: "/roadmap", icon: Map, color: "#f59e0b", label: "Dream Map", sub: roadmapCount > 0 ? `${roadmapCount} steps` : "Generate" },
    { href: "/future-self", icon: Sparkles, color: "#f97316", label: "Future Self", sub: hasFutureSelf ? "View letter" : "Generate" },
    { href: "/tasks", icon: CheckSquare, color: "#f59e0b", label: "Daily Quest", sub: taskCount > 0 ? `${completedTaskCount}/${taskCount} done` : "Add tasks" },
    { href: "/squads", icon: Compass, color: "#f97316", label: "Discover", sub: "Find your people" },
  ];

  const stats = [
    { value: streak, label: "Day Streak", color: "#f59e0b", icon: Flame },
    { value: xp, label: "Total XP", color: "#f97316", icon: Zap },
        ...
        <div className="dash-inner">
          ...
          <div className="sr" style={{ marginBottom: "20px" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "16px" }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <h1 className="welcome-name">Welcome back, <span style={{ color: "#f59e0b" }}>{userName?.split(" ")[0]}</span></h1>
                {currentGoal && (
                  <p className="welcome-goal">
                    <Target size={14} style={{ color: "#f59e0b" }} />{currentGoal}
          ...
          <div className="qa-grid sr sr-d2">
            {quickActions.map((qa) => (
              <Link key={qa.href} href={qa.href} className="qa-card">
                ...
                <ArrowRight size={14} style={{ color: "#3a4458", flexShrink: 0 }} />
              </Link>
            ))}
          </div>
          ...
          <div className="squad-section sr sr-d4">
            <div className="squad-header">
              <p className="squad-title"><Users size={18} style={{ color: "#f59e0b" }} />My Squads</p>
            ...
            {mySquads.length > 0 ? (
              ...
              <div className="empty-wrap">
                <div style={{ display: "flex", justifyContent: "center", marginBottom: "12px" }}>
                  <Users size={40} style={{ color: "#2a3448" }} />
                </div>
