...
export function JoinButton({ squadId }: { squadId: string }) {
  ...
  return (
    ...
    >
      {loading ? (
        <span style={{ width: "18px", height: "18px", border: "2px solid rgba(0,0,0,0.3)", borderTopColor: "#0a0a0a", borderRadius: "50%", animation: "spin 0.6s linear infinite", display: "inline-block" }} />
      ) : (
        <>
          <Sparkles size={16} />
          Join Squad
