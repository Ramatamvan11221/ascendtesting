...
export default async function SquadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isMember && squad.rooms.length === 0) {
    return (
      <AppLayout>
        <SquadHub
          squadId={squad.id} squadName={squad.name} squadIcon={squad.icon}
          squadGoal={squad.goal} squadCategory={squad.category}
          squadDescription={squad.description || ""} memberCount={squad.memberCount}
          createdAt={squad.createdAt.toISOString()} ownerName={squad.owner.name}
          isMember={true} isOwner={isOwner} isModerator={isModerator}
          currentUserId={session?.id || ""} currentUserName={session?.name || "You"}
